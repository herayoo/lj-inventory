...
$(document).on('click', '#craft-btn', function(e){
    ...
    $.post("https://lj-inventory/OpenCraft", JSON.stringify({}));
});
            ...
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    ...
                    $(".inv-options-list").append(
                        '<div class="inv-option-item" id="weapon-attachments"><p>Eklentiler</p></div>'
                    );
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    $(".weapon-attachments-title").html(
                        '<span style="font-weight: bold; letter-spacing: .1vh;">Eklentiler</span>'
                    );
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post(
            ...
            JSON.stringify({
                message: "Bu silah için eklentiler mevcut değil.",
                type: "error",
            })
        );
    }
...
function FormatItemInfo(itemData, dom) {
    ...
    if (itemData != null && itemData.info != "") {
        ...
        //    );
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Sahip: </strong>'+itemData.info.owner+'</p><strong>Süre:</strong> '+itemData.info.cardtime+' dakika</p>'); 
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Sahip: </strong>'+itemData.info.owner+'</p><strong>Süre:</strong> '+itemData.info.cardtime+' minutes</p>'); 
            $(".item-info-stats").html('<p>Weight: '+((itemData.weight * itemData.amount) / 1000).toFixed(1) +' | Amount: '+itemData.amount+ ' | Quality: '+itemData.info.quality.toFixed(0)+'%</p>') 
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Sahip: </strong>'+itemData.info.owner+'</p><strong>Oyun Süresi:</strong> '+itemData.info.cardtime+' dakika</p>'); 
            $(".item-info-stats").html('<p>Ağırlık: '+((itemData.weight * itemData.amount) / 1000).toFixed(1) +' | Miktar: '+itemData.amount+ ' | Kalite: '+itemData.info.quality.toFixed(0)+'%</p>') 
        } else if (itemData.name == "arcadegreen") {
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Sahip: </strong>'+itemData.info.owner+'</p><strong>Oyun Süresi:</strong> '+itemData.info.cardtime+' dakika</p>'); 
            $(".item-info-stats").html('<p>Ağırlık: '+((itemData.weight * itemData.amount) / 1000).toFixed(1) +' | Miktar: '+itemData.amount+ ' | Kalite: '+itemData.info.quality.toFixed(0)+'%</p>') 
            ...
            $(".item-info-description").html('<p>Plaka : ' + itemData.info.plate + '</p></br><p>Model: ' + itemData.info.model + '</p>');
            ...
            $(".item-info-title").html('<p>' + itemData.label + '</p>')
            $(".item-info-description").html('<p><strong>Plaka: </strong><span>'+ itemData.info.label + '</span></p>');
...
(() => {
    ...
    Inventory.Open = function(data) {
        totalWeight = 0;
        totalWeightOther = 0;
        //$("#player-inv-label").html("<i style='color: white; font-style: normal;'>"+" "+data.para+" "+" "+data.para1+"</i>")//
        $("#player-inv-label").html("<i style='color: white; font-style: normal;'>"+" "+data.para+"] - <i class='fas fa-money-bill-alt' style='color: green; font-style: normal;'></i>"+"<i style='color: green; font-style: normal;'>"+" "+data.paraa+"</i>")
    ...
    Inventory.ToggleHotbar = function(data) {
        if (data.open) {
            ...
            var elem =
                '<div class="z-hotbar-item-slot" data-zhotbarslot="5"> <div class="z-hotbar-item-slot-key"><p>6 <i style="top: -62px; left: 58px;"<div class="z-hotbar-item-slot-img"></div><div class="z-hotbar-item-slot-label"><p>&nbsp;</p></div></div>';
            $(".z-hotbar-inventory").append(elem);
                ...
                if (item != null) {
                    ...
                    if (item.slot == 5) {
                        $(".z-hotbar-inventory")
                            .find("[data-zhotbarslot=" + item.slot + "]")
                            .html(
                                '<div class="z-hotbar-item-slot-key"><p>5 <i style="top: -62px; left: 58px;" class=""></i></p></div><div class="z-hotbar-item-slot-img"><img src="images/' +
                                item.image +
                                '" alt="' +
                                item.name +
    ...
    Inventory.UseItem = function(data) {
        ...
        $("#itembox-action").html("<p>Kullanıldı 1x</p>");
    ...
    Inventory.itemBox = function(data) {
        ...
        var type = "Kullanıldı " + data.itemAmount + "x";
        if (data.type == "add") {
          type = "Alındı " + data.itemAmount + "x";
        } else if (data.type == "remove") {
          type = "Silindi " + data.itemAmount + "x";
        }
        ...
        $itembox.fadeIn(50);
        setTimeout(function() {
            $.when($itembox.fadeOut(50)).done(function() {
                $itembox.remove();
            });
        }, 3000);
    };
...
$("#item-give").droppable({
    ...
    drop: function (event, ui) {
        ...
        $.post(`https://${GetParentResourceName()}/GiveItem0`, JSON.stringify({
            inventory: fromInventory, item: fromData, amount: parseInt(amount)
        }), function (state) {
            $('.GiveItemPlayers').html("")
            state.Players.map(dt => $(".GiveItemPlayers").append('<div class="GiveItemPlayersButton" data-mask="'+ dt.isMask +'" data-player="' + dt.id + '">' + dt.props + ' (' + dt.id + ')</div>'))
            $(".GiveItembox").css({ 'display': 'block' });
            $(".GiveItemPlayersButton").click(function () {
                $(".GiveItembox").css({ 'display': 'none' });
                player = $(this).data("player");
                isMask = $(this).data("mask");
