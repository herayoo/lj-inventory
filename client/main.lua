...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    ...
    if not IsEntityDead(PlayerPedId()) then
        if Config.Progressbar.Enable then
            QBCore.Functions.Progressbar('open_inventory', 'Envanter Açılıyor', math.random(Config.Progressbar.minT, Config.Progressbar.maxT), false, true, { -- Name | Label | Time | useWhileDead | canCancel
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", "Üretiliyor..", (math.random(2000, 5000) * amount), false, true, {
		...
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify("Başarısız işlem", "error")
        isCrafting = false
	end)
end)
...
RegisterNetEvent('inventory:client:PickupSnowballs', function()
    ...
    QBCore.Functions.Progressbar("pickupsnowball", "Kartopu alınıyor..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterCommand('inventory', function()
    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["kelepce"] and not PlayerData.metadata["pkelepce"] and not IsPauseMenuActive() then
            local ped = PlayerPedId()
            local curVeh = nil
...
for i = 1, 6 do
    RegisterCommand('slot' .. i,function()
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["kelepce"] and not PlayerData.metadata["pkelepce"] and not IsPauseMenuActive() and not LocalPlayer.state.inv_busy then
            if i == 6 then
