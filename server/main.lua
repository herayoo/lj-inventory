...
local function AddItem(source, item, amount, slot, info)
	...
	if not itemInfo and not Player.Offline then
		TriggerClientEvent("QBCore:Notify", src, "Eşya geçerli değil.", "error")
		return false
	end
	...
	elseif not Player.Offline then
		TriggerClientEvent("QBCore:Notify", src, "Envanter tamamen dolu.", "error")
	end
	return false
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QBCore:Notify", source, "Eşyaya sahip değilsin!", "error")
		return
	end
end
...
--[[ 				secondInv.label = "Stash-"..id ]]
				secondInv.label = '<i class="fas fa-box-open"></i>'.." Depo: "..'<span style="color:#d80032; font-weight: bold;">'..id.."</span>".." "
				secondInv.maxweight = maxweight
				secondInv.inventory = {}
				secondInv.slots = slots
				if Stashes[id] and Stashes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Depo Erişilemiyor"
...
--[[ 				secondInv.label = "Bagaj - Plaka: "..id ]]
				secondInv.label = '<i class="fas fa-truck-loading"></i>'.." Bagaj - Plaka: "..'<span style="color:#d80032; font-weight: bold;">'..id.."</span>".." "
				...
				if (Trunks[id] and Trunks[id].isOpen) or (QBCore.Shared.SplitStr(id, "PLZI")[2] and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
					secondInv.label = "Bagaj Kullanılamıyor"
					secondInv.maxweight = other.maxweight or 60000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
			...
			elseif name == "glovebox" then
				...
				secondInv.label = '<i class="fas fa-truck-loading"></i>'.." Torpido - Plaka: "..'<span style="color:#d80032; font-weight: bold;">'..id.."</span>".." "
				secondInv.maxweight = 10000
				secondInv.inventory = {}
				secondInv.slots = 5
				if Gloveboxes[id] and Gloveboxes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Torpido Kullanılamıyor"
					secondInv.maxweight = 10000
					secondInv.inventory = {}
					secondInv.slots = 0
				else
...
--[[ 				secondInv.label = ""..other.label..'<span style="color:#64e778;''- Nakit: '..para.."$</span>" ]]
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] and not Drops[id].isOpen then
					...
					secondInv.name = id
					secondInv.label = '<i class="fas fa-box-open"></i>'.." Zemin-"..tostring(id)
		...
		TriggerClientEvent('QBCore:Notify', src, 'Şu anda envantere erişemezsin.', 'error')
	end
end)
...
--[[ RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				...
				if IsItemValid then
					...
					TriggerClientEvent('QBCore:Notify', src, "Bu eşyayı satamazsın..", 'error')
				end
			else
				-- drop
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('QBCore:Notify', src, "Gerekli olan eşyalara sahip değilsin..", "error")
		end
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				Player.Functions.AddItem(itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('gct-shops:client:UpdateShop', src, QBCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("qb-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
			elseif bankBalance >= price then
				Player.Functions.RemoveMoney("bank", price, "itemshop-bought-item")
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot, label)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	target = tonumber(target)
    local OtherPlayer = QBCore.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return TriggerClientEvent("QBCore:Notify", src, "Kendine eşya veremezsin", "error") end
	if dist > 2 then return TriggerClientEvent("QBCore:Notify", src, "Eşya verebilmek için çok uzaktasın", "error") end
	local item = GetItemBySlot(src, slot)
	if not item then TriggerClientEvent("QBCore:Notify", src, "Vermeye çalıştığın eşya bulunamadı", "error"); return end
	if item.name ~= name then TriggerClientEvent("QBCore:Notify", src, "Geçersiz eşya bulundu, tekrar deneyin", "error"); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				TriggerClientEvent(target, "QBCore:Notify", Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." kişisinden "..amount.." adet "..item.label.." aldın")
				...
				QBCore.Functions.Notify(target, "Diğer oyuncunun envanteri dolu!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
...
QBCore.Commands.Add("resetinv", "Envanter Sıfırla (Admin)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID / stash / plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "Geçerli tip değil..", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Boşluklar doğru bir şekilde doldurulmalı..", "error")
	end
end, "admin")

QBCore.Commands.Add("rob", "Oyuncuyu Soy", {}, false, function(source, args)
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, GetPlayerName(id)..' kişisine '..amount..' adet '..itemData["name"]..' verdin.', "success")
				else
					TriggerClientEvent('QBCore:Notify', source, 'Eşya verilemiyor.', "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source, 'Eşya geçersiz.', "error")
			end
	else
...
QBCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    ...
    if Player then
        ...
        QBCore.Functions.Notify(source, "Oyuncu aktif değil", 'error')
    end
end, 'admin')
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			TriggerClientEvent('chat:addMessage', v,  {
					args = { 'Sürücü Ehliyeti', '\n'..item.info.firstname..' '..item.info.lastname..' | '..item.info.birthdate..' | '..item.info.type}
				}
			)
		end
	end
...
CreateUsableItem("id_card", function(source, item)
	...
	for _, v in pairs(players) do
		...
		if dist < 3.0 then
			...
			TriggerClientEvent('chat:addMessage', v,  {
				args = { 'Kimlik', '\n'..item.info.citizenid..' | '..item.info.firstname..' '..item.info.lastname..' | '..item.info.birthdate..' | '..gender..' | '..item.info.nationality}
			}
			)
		end
	end
